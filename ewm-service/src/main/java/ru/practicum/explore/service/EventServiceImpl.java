...
public class EventServiceImpl implements EventService {
    ...
    public List<EventShortDto> getPublicEvents(String text, List<Long> categories, Boolean paid,
        ...
        statsClient.hit(new StatsDtoInput("ewm-main-server", uri, ((HttpServletRequest) request).getRemoteAddr(),
                LocalDateTime.now().format(StatsDtoInput.DATE_TIME_FORMATTER)));

        List<StatsDtoOutput> statsList = statsClient.getStats(
        ...
        for (Event event : events) {
            Optional<StatsDtoOutput> stat = statsList.stream()
                    .filter(s -> s.getApp().equals("ewm-main-server") && s.getUri().endsWith("/events/" + event.getId()))
                    .findFirst();
            event.setViews(stat.map(StatsDtoOutput::getHits).orElse(0L));
        }

        return events.stream()
                .filter(e -> !onlyAvailable || e.getConfirmedRequests() < e.getParticipantLimit())
                .sorted(sort == null ?
                        Comparator.comparing(Event::getEventDate) :
                        Comparator.comparing(Event::getViews).reversed())
                .skip(from)
                .limit(size)
                .map(e -> {
    ...
    private Long getViews(Long eventId) {
        List<StatsDtoOutput> stats = statsClient.getStats(
                LocalDateTime.now().minusYears(10),
                LocalDateTime.now().plusYears(10),
